# CKKS bootstrapping, S2C-first — the AVX2 / FMA + Rayon submission

The harness as `fherma implementation init ckks-bootstrapping/s2c@1.0.0` writes
it, with two lines changed: `Cargo.toml` selects the `avx-rayon` backend
(AVX2 with FMA across worker threads) and `config.jsonc` asks for 24 threads.
`src/run.rs` is the reference's, untouched: this row competes on backend and
threads, not on the algorithm.

Poulpy publishes 3.26 s at 16 threads for this backend on a Ryzen 9 9950X. The number here
